...
const MetasVendedoras = (props) => {
    ...
    useEffect(() => {
        api.get(`metavendedorageralvend/${dataHoje}/${dataHoje}/${loja}/${ven_numero}/`).then(response => {
    ...
    const consultasdiversas = useCallback(() => {
        api.get(`metavendedorageralvend/${datainicial}/${datafinal}/${loja}/${ven_numero}/`).then(response => {
            const uniqueProgresso = response.data.filter((value, index, self) => {
                return self.indexOf(value) === index;
            });
            setProgresso(uniqueProgresso);
